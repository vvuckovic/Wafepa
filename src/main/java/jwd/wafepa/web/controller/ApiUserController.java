...
public class ApiUserController {
	...
	ResponseEntity<List<UserDTO>> getUser(
			...
			@RequestParam(value = "property", required = false, defaultValue="firstName") String property,
		...
		if (firstname != null) {
		Page<User> usersPage = userService.findByFirstNameContainsOrLastNameContains(page, firstname, lastname);
		users = usersPage.getContent();
		totalPages=usersPage.getTotalPages();
		totalEl=usersPage.getTotalElements();
		}else{
			Page<User> usersPage = userService.findAll(page, Sort.Direction.fromString(direction), property);
			users=usersPage.getContent();
			totalPages=usersPage.getTotalPages();
			totalEl=usersPage.getTotalElements();
		}
		HttpHeaders httpHeader = new HttpHeaders();
		httpHeader.add("total-Upages", "" + totalPages);
		httpHeader.add("total-Uelements", ""+totalEl);
		
		return new ResponseEntity<>(toDto.convert(users),httpHeader, HttpStatus.OK);
	}
